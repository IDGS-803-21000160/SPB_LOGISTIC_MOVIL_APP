...
const postInsertBigTicketRoutes = async (data) => {
  try {
    const response = await axios.post(
      `${API_BASE_URL}/routes/insertBigTicketRoutes`,
      data
    );
    return response.data;
  } catch (error) {
    console.error("Error posting big ticket routes:", error);
    throw error;
  }
};

export { postAddRoute, postConvertSharedRoute, postInsertBigTicketRoutes };
